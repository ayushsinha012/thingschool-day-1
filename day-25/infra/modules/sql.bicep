...
// server at all, so no secret of any kind is needed to stand it up, and
// no secret can ever exist for it later (azureADOnlyAuthentication: true
// disables SQL authentication outright, it is not just unused).
