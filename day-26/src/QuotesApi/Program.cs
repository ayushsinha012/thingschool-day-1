...
app.Logger.LogInformation(
    "Azure Monitor OpenTelemetry export enabled: {Enabled}",
    InfrastructureExtensions.IsAzureMonitorConfigured(builder.Configuration));
...
// Enforces the "auth" fixed-window policy from
// InfrastructureExtensions.AddAuthRateLimiting - per-IP throttling on the
// login/register endpoints (see [EnableRateLimiting("auth")] on
// AuthController).
app.UseRateLimiter();
