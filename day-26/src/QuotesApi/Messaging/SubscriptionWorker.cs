    ...
    string workerSlotPrefix) : BackgroundService
{
    ...
    private async Task HandleMessageAsync(ProcessMessageEventArgs args)
    {
        ...
        var parentContext = message.ApplicationProperties.TryGetValue(
            MessagingTelemetry.TraceParentProperty,
            out var traceParentValue) && traceParentValue is string traceParent
            ? traceParent
            : null;

        var traceState = message.ApplicationProperties.TryGetValue(
            MessagingTelemetry.TraceStateProperty,
            out var traceStateValue) && traceStateValue is string traceStateString
            ? traceStateString
            : null;

        using var activity = MessagingTelemetry.ActivitySource.StartActivity(
            $"{_options.TopicName} process",
            ActivityKind.Consumer,
            parentContext ?? string.Empty);
        ...
        activity?.SetTag("messaging.destination.name", _options.TopicName);
