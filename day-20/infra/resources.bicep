...
@description('The deployed Static Web App origin (Day 17), allow-listed for browser CORS - never a wildcard. Empty until the real Static Web App exists.')
    ...
    containers: [
      {
        ...
        // Binds to Cors:ProductionOrigins:0 (see
        // Extensions/InfrastructureExtensions.cs) - the real Day 17 Static
        // Web App origin, added as an explicit allow-listed entry, never a
        // wildcard. Conditionally included so an empty corsProductionOrigin
        // (the param's default, before the Static Web App existed) doesn't
        // add a blank origin to the list.
        empty(corsProductionOrigin) ? [] : [
          {
