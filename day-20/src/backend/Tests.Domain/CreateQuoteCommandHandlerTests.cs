...
public class CreateQuoteCommandHandlerTests
{
    ...
    public async Task Handle_WithValidAuthorAndText_PersistsQuoteAndReturnsResult()
    {
        ...
        repository.OutboxMessages.Should().ContainSingle();
        repository.OutboxMessages[0].EventType.Should().Be("quote.created");
        repository.OutboxMessages[0].Payload.Should().Contain("Marcus Aurelius");
    }
